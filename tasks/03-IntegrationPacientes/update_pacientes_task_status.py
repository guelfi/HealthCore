...
def main():
    if len(sys.argv) < 3:
        color_log("Uso: python update_pacientes_task_status.py <task_id> <status> [notes]", Colors.YELLOW)
        color_log("Exemplo: python update_pacientes_task_status.py validate_backend_endpoints COMPLETE \"Endpoints validados\"", Colors.GRAY)
    ...
    json_file = "tasks/03-IntegrationPacientes/integration_pacientes_001.json"
    ...
    color_log("  python tasks/03-IntegrationPacientes/view_pacientes_session_status.py", Colors.GRAY)

    if next_task:
        color_log("\nIniciar próxima tarefa:", Colors.WHITE)
        color_log(f"  python tasks/03-IntegrationPacientes/update_pacientes_task_status.py \"{next_task.get('id', 'N/A')}\" IN_PROGRESS \"Iniciando desenvolvimento\"", Colors.GRAY)

    # Verificar se a etapa está completa
    if completion_percentage == 100:
        color_log("\n🎉 PARABÉNS! INTEGRAÇÃO DE PACIENTES 100% CONCLUÍDA!", Colors.GREEN)
        color_log("🚀 Pronto para próxima etapa: IntegrationExames", Colors.GREEN)
