#!/usr/bin/env python3
"""
Script para atualizar status de tarefas na integração de Usuários
...
def main():
    if len(sys.argv) < 3:
        color_log("Uso: python update_usuarios_task_status.py <task_id> <status> [notes]", Colors.YELLOW)
        color_log("Exemplo: python update_usuarios_task_status.py validate_users_endpoints COMPLETE \"Endpoints validados\"", Colors.GRAY)
    ...
    color_log("  python tasks/05-IntegrationUsuarios/view_usuarios_session_status.py", Colors.GRAY)

    if next_task:
        color_log("\nIniciar próxima tarefa:", Colors.WHITE)
        color_log(f"  python tasks/05-IntegrationUsuarios/update_usuarios_task_status.py \"{next_task.get('id', 'N/A')}\" IN_PROGRESS \"Iniciando desenvolvimento\"", Colors.GRAY)

    # Verificar se a etapa está completa
    if completion_percentage == 100:
        color_log("\n🎉 PARABÉNS! INTEGRAÇÃO DE USUÁRIOS 100% CONCLUÍDA!", Colors.GREEN)
        color_log("🚀 Pronto para próxima etapa: IntegrationMedicos", Colors.GREEN)
