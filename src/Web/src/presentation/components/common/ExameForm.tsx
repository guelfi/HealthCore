...
const ExameForm: React.FC<ExameFormProps> = ({
  ...
  useEffect(() => {
    ...
    // Limpar erros
    setFormError(null);
  }, [mode, initialData, setValue, pacientes, isEditing, reset]);
  ...
  return (
    <>
        ...
        >
          ...
          <Box display="flex" gap={2} justifyContent="flex-end" mt={1}>
            <StandardDialogButtons
              ...
              onDelete={isEditing && onDelete && initialData ? () => onDelete(initialData.id) : undefined}
              showDelete={!!(isEditing && onDelete && initialData)}
              saveLoading={loading}
              saveText={isEditing ? 'Salvar' : 'Adicionar'}
            />
          </Box>
        </Box>
      </Box>
      ...
      >
        ...
        <StandardDialogButtons
          onSave={() => setShowPacienteDialog(false)}
          onClose={() => setShowPacienteDialog(false)}
          saveText="Fechar"
        />
      </Dialog>
