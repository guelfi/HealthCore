...
export const useExames = (): UseExamesState & UseExamesActions => {
  ...
  const fetchExames = useCallback(async (params: ExameQueryParams = {}) => {
    setLoading(true);
    clearError();
  ...
  const deleteExame = useCallback(async (id: string): Promise<void> => {
    setLoading(true);
    clearError();
  ...
  const getExamesByPaciente = useCallback(async (pacienteId: string, params: Omit<ExameQueryParams, 'pacienteId'> = {}) => {
    setLoading(true);
    clearError();
  ...
  const getExamesByModalidade = useCallback(async (modalidade: ModalidadeDicom, params: Omit<ExameQueryParams, 'modalidade'> = {}) => {
    setLoading(true);
    clearError();
  ...
  const getExamesByPeriodo = useCallback(async (dataInicio: Date, dataFim: Date, params: Omit<ExameQueryParams, 'dataInicio' | 'dataFim'> = {}) => {
    setLoading(true);
    clearError();
    
    try {
      const response = await ExameService.getByPeriodo(dataInicio, dataFim, params);
      updateExamesState(response);
    } catch (error: any) {
      const errorMessage = error.response?.data?.message || 
                          error.response?.data?.error ||
  ...
  const checkIdempotency = useCallback(async (idempotencyKey: string): Promise<boolean> => {
    try {
