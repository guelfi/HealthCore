...
| R-002 | Critical | Previously committed JWT fallback/private material may remain in history | Invalidate sessions, rotate secrets, rewrite history | Open; SSH closed 2026-07-29; JWT/app decision pending |
...
| R-007 | High | GitHub workflow has not run under repository credentials | Re-authenticate the intended GitHub account and run CI | Mitigated: OCI CD succeeded on HealthCore after 2026-07-29 secret updates |
...
| R-011 | High | Local SSH key could be committed or copied into images | Rotated to `oci-key-2026-07-29`; old OCI authorized keys revoked; HealthCore `OCI_SSH_PRIVATE_KEY` updated | Mitigated for live SSH; historical objects still Gate B rewrite scope |
